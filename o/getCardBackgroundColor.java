...
final class getCardBackgroundColor {
    ...
    static void setMax(o.evictAll r8, o.resize r9) {
        /*
            r0 = 0
            r1 = 0
            r2 = r1
        ...
        L_0x003a:
            return
        */
        throw new UnsupportedOperationException("Method not decompiled: o.getCardBackgroundColor.setMax(o.evictAll, o.resize):void");
    }

    static int setMax(evictAll evictall) {
        long j = 0;
        for (int i = 0; i < evictall.j(); i++) {
            j += (long) setMin[evictall.b(i) & 255];
        }
        return (int) ((j + 7) >> 3);
    }
    ...
    public final byte[] setMax(byte[] bArr) {
        ...
        for (byte b2 : bArr) {
            ...
            i += 8;
            while (i >= 8) {
                setmin = setmin.setMax[(b >>> (i - 8)) & 255];
                if (setmin.setMax == null) {
                    byteArrayOutputStream.write(setmin.getMin);
                    i -= setmin.getMax;
                    setmin = this.getMin;
                } else {
                    i -= 8;
        ...
        while (i > 0) {
            setMin setmin2 = setmin.setMax[(b << (8 - i)) & 255];
            if (setmin2.setMax != null || setmin2.getMax > i) {
                break;
            }
            byteArrayOutputStream.write(setmin2.getMin);
            i -= setmin2.getMax;
            setmin = this.getMin;
        }
    ...
    private void length() {
        ...
        while (true) {
            ...
            if (i < bArr.length) {
                int i2 = setMax[i];
                byte b = bArr[i];
                setMin setmin = new setMin(i, b);
                setMin setmin2 = this.getMin;
                while (b > 8) {
                    b = (byte) (b - 8);
                    int i3 = (i2 >>> b) & 255;
                    if (setmin2.setMax != null) {
                        if (setmin2.setMax[i3] == null) {
                            setmin2.setMax[i3] = new setMin();
                        }
                        setmin2 = setmin2.setMax[i3];
                    } else {
