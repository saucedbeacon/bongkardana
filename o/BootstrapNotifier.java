...
public final class BootstrapNotifier extends parseFailed<Boolean, Unit> {
    private final isPacketDistinct twilioSdkRepository;

    /* JADX INFO: super call moved to the top of the method (can break code semantics) */
    @Inject
    public BootstrapNotifier(@NotNull appxLoadFailed appxloadfailed, @NotNull getScheme getscheme, @NotNull isPacketDistinct ispacketdistinct) {
    ...
    public final TitleBarRightButtonView.AnonymousClass1<Boolean> buildUseCaseObservable(@Nullable Unit unit) {
        ...
        TitleBarRightButtonView.CornerMarkingUIController.AnonymousClass1<Boolean> deleteLocalFactor = this.twilioSdkRepository.deleteLocalFactor();
        if (deleteLocalFactor instanceof getSecondFloorView) {
            r2 = ((getSecondFloorView) deleteLocalFactor).setMax();
        } else {
            setTitleLoading settitleloading = new setTitleLoading(deleteLocalFactor);
        ...
        Intrinsics.checkNotNullExpressionValue(r2, "twilioSdkRepository.dele…alFactor().toObservable()");
        return r2;
    }
}
