...
public final class onNonBeaconLeScan extends parseFailed<Boolean, Unit> {
    private final isPacketDistinct twilioSdkRepository;

    /* JADX INFO: super call moved to the top of the method (can break code semantics) */
    @Inject
    public onNonBeaconLeScan(@NotNull appxLoadFailed appxloadfailed, @NotNull getScheme getscheme, @NotNull isPacketDistinct ispacketdistinct) {
    ...
    public final TitleBarRightButtonView.AnonymousClass1<Boolean> buildUseCaseObservable(@Nullable Unit unit) {
        ...
        TitleBarRightButtonView.CornerMarkingUIController.AnonymousClass1<Boolean> checkEnrollmentStatus = this.twilioSdkRepository.checkEnrollmentStatus();
        if (checkEnrollmentStatus instanceof getSecondFloorView) {
            r2 = ((getSecondFloorView) checkEnrollmentStatus).setMax();
        } else {
            setTitleLoading settitleloading = new setTitleLoading(checkEnrollmentStatus);
        ...
        Intrinsics.checkNotNullExpressionValue(r2, "twilioSdkRepository.chec…ntStatus().toObservable()");
        return r2;
    }
}
