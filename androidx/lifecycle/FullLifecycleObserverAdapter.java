...
public class FullLifecycleObserverAdapter implements setPopupCallback {
    private final setCheckable getMax;
    private final setPopupCallback length;

    public FullLifecycleObserverAdapter(setCheckable setcheckable, setPopupCallback setpopupcallback) {
        this.getMax = setcheckable;
        this.length = setpopupcallback;
    }
    ...
    public void setMax(@NonNull setChecked setchecked, @NonNull Lifecycle.Event event) {
        if (AnonymousClass4.setMin[event.ordinal()] != 7) {
            setPopupCallback setpopupcallback = this.length;
            if (setpopupcallback != null) {
                setpopupcallback.setMax(setchecked, event);
                return;
            }
            return;
        }
