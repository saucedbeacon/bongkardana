...
public class SQLiteEventStore implements EventStore, SynchronizationGuard {
    ...
    public SQLiteDatabase getDb() {
        ...
        return (SQLiteDatabase) retryIfDbLocked(new isAdded(schemaManager2), isInLayout.setMin);
    }
    ...
    public PersistedEvent persist(TransportContext transportContext, EventInternal eventInternal) {
        ...
        long longValue = ((Long) inTransaction(new getRetainInstance(this, transportContext, eventInternal))).longValue();
        if (longValue < 1) {
            return null;
        }
    ...
    public /* synthetic */ Long lambda$persist$1(TransportContext transportContext, EventInternal eventInternal, SQLiteDatabase sQLiteDatabase) {
        if (isStorageAtLimit()) {
            return -1L;
        }
    ...
    private Long getTransportContextId(SQLiteDatabase sQLiteDatabase, TransportContext transportContext) {
        ...
        return (Long) tryWithCursor(sQLiteDatabase.query("transport_contexts", new String[]{FieldType.FOREIGN_ID_FIELD_SUFFIX}, sb.toString(), (String[]) arrayList.toArray(new String[0]), (String) null, (String) null, (String) null), getUserVisibleHint.getMin);
    }
    ...
    public void recordFailure(Iterable<PersistedEvent> iterable) {
        if (iterable.iterator().hasNext()) {
            ...
            inTransaction(new requestPermissions(sb.toString()));
        }
    }
    ...
    public long getNextCallTime(TransportContext transportContext) {
        return ((Long) tryWithCursor(getDb().rawQuery("SELECT next_request_ms FROM transport_contexts WHERE backend_name = ? and priority = ?", new String[]{transportContext.getBackendName(), String.valueOf(PriorityMapping.toInt(transportContext.getPriority()))}), getLoaderManager.getMin)).longValue();
    }
    ...
    public boolean hasPendingEventsFor(TransportContext transportContext) {
        return ((Boolean) inTransaction(new startActivity(this, transportContext))).booleanValue();
    }

    public /* synthetic */ Boolean lambda$hasPendingEventsFor$5(TransportContext transportContext, SQLiteDatabase sQLiteDatabase) {
        Long transportContextId = getTransportContextId(sQLiteDatabase, transportContext);
        if (transportContextId == null) {
            return Boolean.FALSE;
        }
        return (Boolean) tryWithCursor(getDb().rawQuery("SELECT 1 FROM events WHERE context_id = ? LIMIT 1", new String[]{transportContextId.toString()}), setHasOptionsMenu.getMax);
    }

    public void recordNextCallTime(TransportContext transportContext, long j) {
        inTransaction(new setUserVisibleHint(j, transportContext));
    }
    ...
    public Iterable<PersistedEvent> loadBatch(TransportContext transportContext) {
        return (Iterable) inTransaction(new onInflate(this, transportContext));
    }

    public /* synthetic */ List lambda$loadBatch$7(TransportContext transportContext, SQLiteDatabase sQLiteDatabase) {
    ...
    public Iterable<TransportContext> loadActiveContexts() {
        return (Iterable) inTransaction(onAttachFragment.getMin);
    }

    public static /* synthetic */ List lambda$loadActiveContexts$9(SQLiteDatabase sQLiteDatabase) {
        return (List) tryWithCursor(sQLiteDatabase.rawQuery("SELECT distinct t._id, t.backend_name, t.priority, t.extras FROM transport_contexts AS t, events AS e WHERE e.context_id = t._id", new String[0]), onHiddenChanged.setMax);
    }
    ...
    public int cleanUp() {
        return ((Integer) inTransaction(new isMenuVisible(this.wallClock.getTime() - this.config.getEventCleanUpAge()))).intValue();
    }
    ...
    public void clearDb() {
        inTransaction(hasOptionsMenu.setMin);
    }
    ...
    private List<PersistedEvent> loadEvents(SQLiteDatabase sQLiteDatabase, TransportContext transportContext) {
        ...
        tryWithCursor(sQLiteDatabase2.query("events", new String[]{FieldType.FOREIGN_ID_FIELD_SUFFIX, "transport_name", "timestamp_ms", "uptime_ms", "payload_encoding", "payload", "code", "inline"}, "context_id = ?", new String[]{transportContextId.toString()}, (String) null, (String) null, (String) null, String.valueOf(this.config.getLoadBatchSize())), new isVisible(this, arrayList, transportContext));
        return arrayList;
    }

    public /* synthetic */ Object lambda$loadEvents$12(List list, TransportContext transportContext, Cursor cursor) {
        while (cursor.moveToNext()) {
            boolean z = false;
    ...
    private byte[] readPayload(long j) {
        return (byte[]) tryWithCursor(getDb().query("event_payloads", new String[]{"bytes"}, "event_id = ?", new String[]{String.valueOf(j)}, (String) null, (String) null, "sequence_num"), isHidden.setMax);
    }
    ...
    private Map<Long, Set<Metadata>> loadMetadata(SQLiteDatabase sQLiteDatabase, List<PersistedEvent> list) {
        ...
        sb.append(')');
        tryWithCursor(sQLiteDatabase.query("event_metadata", new String[]{"event_id", "name", "value"}, sb.toString(), (String[]) null, (String) null, (String) null, (String) null), new isResumed(hashMap));
        return hashMap;
    }
    ...
    private void ensureBeginTransaction(SQLiteDatabase sQLiteDatabase) {
        retryIfDbLocked(new setMenuVisibility(sQLiteDatabase), setRetainInstance.length);
    }
