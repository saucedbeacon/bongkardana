...
public class MaterialButtonToggleGroup extends LinearLayout {
        ...
        if (view instanceof MaterialButton) {
            ...
            ViewCompat.getMin((View) materialButton, (setOnCancelListener) new setOnCancelListener() {
                public void onInitializeAccessibilityNodeInfo(View view, @NonNull onSupportActionModeFinished onsupportactionmodefinished) {
                    ...
                    onsupportactionmodefinished.length((Object) onSupportActionModeFinished.setMin.getMin(0, 1, MaterialButtonToggleGroup.this.getIndexWithinVisibleButtons(view), 1, false, ((MaterialButton) view).isChecked()));
                }
            });
        }
    }
    ...
    public void onInitializeAccessibilityNodeInfo(@NonNull AccessibilityNodeInfo accessibilityNodeInfo) {
        ...
        onSupportActionModeFinished.getMin(accessibilityNodeInfo).getMin((Object) onSupportActionModeFinished.getMax.length(1, getVisibleButtonCount(), false, isSingleSelection() ? 1 : 2));
    }
    ...
    private void adjustChildMarginsAndUpdateLayout() {
        ...
        if (firstVisibleChildIndex != -1) {
            for (int i = firstVisibleChildIndex + 1; i < getChildCount(); i++) {
                ...
                if (getOrientation() == 0) {
                    setMultiChoiceItems.setMin(buildLayoutParams, 0);
                    setMultiChoiceItems.length(buildLayoutParams, -min);
                    buildLayoutParams.topMargin = 0;
                } else {
                    buildLayoutParams.bottomMargin = 0;
                    buildLayoutParams.topMargin = -min;
                    setMultiChoiceItems.length(buildLayoutParams, 0);
                }
    ...
    private void resetChildMargins(int i) {
        if (getChildCount() != 0 && i != -1) {
            ...
            setMultiChoiceItems.setMin(layoutParams, 0);
            setMultiChoiceItems.length(layoutParams, 0);
    ...
    private void setGeneratedIdIfNeeded(@NonNull MaterialButton materialButton) {
        if (materialButton.getId() == -1) {
            materialButton.setId(ViewCompat.getMin());
        }
    }
