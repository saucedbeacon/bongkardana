...
public class FloatingActionButton extends VisibilityAwareImageButton implements onStart, closeOptionsMenu, ExpandableTransformationWidget, Shapeable, CoordinatorLayout.getMax {
    ...
    @NonNull
    private final sendMediaButton imageHelper;
    @Nullable
    ...
    public FloatingActionButton(@NonNull Context context, @Nullable AttributeSet attributeSet, int i) {
        ...
        this.imageHelper = sendmediabutton;
    ...
    private void onApplySupportImageTint() {
        ...
        if (drawable != null) {
            ...
            if (colorStateList == null) {
                RequiresPermission.Read.IsOverlapping(drawable);
                return;
            }
            ...
            drawable.mutate().setColorFilter(unregisterCallbackListener.getMax(colorForState, mode));
        }
    }

    public void setImageResource(@DrawableRes int i) {
        this.imageHelper.setMax(i);
        onApplySupportImageTint();
    }
    ...
    public static class Behavior extends BaseBehavior<FloatingActionButton> {
        ...
        public /* bridge */ /* synthetic */ void onAttachedToLayoutParams(@NonNull CoordinatorLayout.length length) {
    ...
    protected static class BaseBehavior<T extends FloatingActionButton> extends CoordinatorLayout.Behavior<T> {
        ...
        private static boolean isBottomSheet(@NonNull View view) {
            ...
            if (layoutParams instanceof CoordinatorLayout.length) {
                return ((CoordinatorLayout.length) layoutParams).setMin instanceof BottomSheetBehavior;
            }
            return false;
        }
        ...
        private boolean shouldUpdateVisibility(@NonNull View view, @NonNull FloatingActionButton floatingActionButton) {
            CoordinatorLayout.length length = (CoordinatorLayout.length) floatingActionButton.getLayoutParams();
            if (this.autoHideEnabled && length.toIntRange == view.getId() && floatingActionButton.getUserSetVisibility() == 0) {
                return true;
            }
            return false;
        }
        ...
        private boolean updateFabVisibilityForBottomSheet(@NonNull View view, @NonNull FloatingActionButton floatingActionButton) {
            ...
            if (view.getTop() < (floatingActionButton.getHeight() / 2) + ((CoordinatorLayout.length) floatingActionButton.getLayoutParams()).topMargin) {
                floatingActionButton.hide(this.internalAutoHideListener, false);
                return true;
            }
            floatingActionButton.show(this.internalAutoHideListener, false);
        ...
        private void offsetIfNeeded(@NonNull CoordinatorLayout coordinatorLayout, @NonNull FloatingActionButton floatingActionButton) {
            ...
            if (rect != null && rect.centerX() > 0 && rect.centerY() > 0) {
                CoordinatorLayout.length length = (CoordinatorLayout.length) floatingActionButton.getLayoutParams();
                int i2 = 0;
                if (floatingActionButton.getRight() >= coordinatorLayout.getWidth() - length.rightMargin) {
                    i = rect.right;
                } else {
                    i = floatingActionButton.getLeft() <= length.leftMargin ? -rect.left : 0;
                }
                if (floatingActionButton.getBottom() >= coordinatorLayout.getHeight() - length.bottomMargin) {
                    i2 = rect.bottom;
                } else if (floatingActionButton.getTop() <= length.topMargin) {
                    i2 = -rect.top;
                }
                if (i2 != 0) {
                    ViewCompat.equals(floatingActionButton, i2);
                }
                if (i != 0) {
                    ViewCompat.toFloatRange(floatingActionButton, i);
                }
            }
        }
    }
