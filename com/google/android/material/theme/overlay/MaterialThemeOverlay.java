...
public class MaterialThemeOverlay {
    ...
    public static Context wrap(@NonNull Context context, @Nullable AttributeSet attributeSet, @AttrRes int i, @StyleRes int i2) {
        ...
        boolean z = (context instanceof MediaBrowserCompat$MediaItem) && ((MediaBrowserCompat$MediaItem) context).length == obtainMaterialThemeOverlayId;
        if (obtainMaterialThemeOverlayId == 0 || z) {
            return context;
        }
        MediaBrowserCompat$MediaItem mediaBrowserCompat$MediaItem = new MediaBrowserCompat$MediaItem(context, obtainMaterialThemeOverlayId);
        int obtainAndroidThemeOverlayId = obtainAndroidThemeOverlayId(context, attributeSet);
        if (obtainAndroidThemeOverlayId != 0) {
            mediaBrowserCompat$MediaItem.getTheme().applyStyle(obtainAndroidThemeOverlayId, true);
        }
