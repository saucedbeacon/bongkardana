    ...
    private void updateImportantForAccessibility(@NonNull View view, boolean z) {
        ...
        if (parent instanceof CoordinatorLayout) {
            ...
            for (int i = 0; i < childCount; i++) {
                ...
                boolean z2 = (childAt.getLayoutParams() instanceof CoordinatorLayout.length) && (((CoordinatorLayout.length) childAt.getLayoutParams()).setMin instanceof FabTransformationScrimBehavior);
                if (childAt != view && !z2) {
                    if (!z) {
                        Map<View, Integer> map = this.importantForAccessibilityMap;
                        if (map != null && map.containsKey(childAt)) {
                            ViewCompat.setMin(childAt, this.importantForAccessibilityMap.get(childAt).intValue());
                        }
                    } else {
