...
public abstract class FabTransformationBehavior extends ExpandableTransformationBehavior {
    ...
    private void createElevationAnimation(View view, @NonNull View view2, boolean z, boolean z2, @NonNull FabTransformationSpec fabTransformationSpec, @NonNull List<Animator> list, List<Animator.AnimatorListener> list2) {
        ...
        float Mean$Arithmetic = ViewCompat.Mean$Arithmetic(view2) - ViewCompat.Mean$Arithmetic(view);
        if (z) {
            if (!z2) {
                view2.setTranslationZ(-Mean$Arithmetic);
            }
            objectAnimator = ObjectAnimator.ofFloat(view2, View.TRANSLATION_Z, new float[]{0.0f});
        } else {
            objectAnimator = ObjectAnimator.ofFloat(view2, View.TRANSLATION_Z, new float[]{-Mean$Arithmetic});
        }
    ...
    private int getBackgroundTint(@NonNull View view) {
        ColorStateList extraCallback = ViewCompat.extraCallback(view);
        if (extraCallback != null) {
            return extraCallback.getColorForState(view.getDrawableState(), extraCallback.getDefaultColor());
        }
        return 0;
    }
