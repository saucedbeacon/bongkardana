...
public class Where<T, ID> {
    ...
    private Where<T, ID> in(boolean z, String str, QueryBuilder<?, ?> queryBuilder) throws SQLException {
        if (queryBuilder.getSelectColumnCount() == 1) {
            ...
            StringBuilder sb = new StringBuilder("Inner query must have only 1 select column specified instead of ");
    ...
    private Clause[] buildClauseArray(Where<T, ID>[] whereArr, String str) {
        ...
        if (length != max) {
            ...
            onCancelLoad.setMax(1384428688, oncanceled);
            onCancelLoad.getMin(1384428688, oncanceled);
        }
        if (whereArr.length == 0) {
            return null;
        }
        Clause[] clauseArr = new Clause[whereArr.length];
        for (int length2 = whereArr.length - 1; length2 >= 0; length2--) {
            clauseArr[length2] = pop(str);
        }
        return clauseArr;
    }
    ...
    private Clause pop(String str) {
        ...
        StringBuilder sb = new StringBuilder("Expecting there to be a clause already defined for '");
        sb.append(str);
        sb.append("' operation");
        throw new IllegalStateException(sb.toString());
    }
