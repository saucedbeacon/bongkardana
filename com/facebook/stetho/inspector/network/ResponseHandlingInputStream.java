...
public final class ResponseHandlingInputStream extends FilterInputStream {
    ...
    private synchronized void closeOutputStreamQuietly() {
        if (!this.mClosed) {
            try {
                ...
                try {
                    CLog.writeToConsole(this.mNetworkPeerManager, Console.MessageLevel.ERROR, Console.MessageSource.NETWORK, "Could not close the output stream".concat(String.valueOf(e)));
                } finally {
                    this.mClosed = true;
                }
            }
    ...
    private void handleIOExceptionWritingToStream(IOException iOException) {
        CLog.writeToConsole(this.mNetworkPeerManager, Console.MessageLevel.ERROR, Console.MessageSource.NETWORK, "Could not write response body to the stream ".concat(String.valueOf(iOException)));
        closeOutputStreamQuietly();
    }
}
