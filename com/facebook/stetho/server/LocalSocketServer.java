...
public class LocalSocketServer {
    ...
    private void listenOnAddress(String str) throws IOException {
        ...
        LogUtil.i("Listening on @".concat(String.valueOf(str)));
        while (!Thread.interrupted()) {
            try {
                WorkerThread workerThread = new WorkerThread(this.mServerSocket.accept(), this.mSocketHandler);
                StringBuilder sb = new StringBuilder("StethoWorker-");
        ...
        LogUtil.i("Server shutdown on @".concat(String.valueOf(str)));
    }
        ...
        while (true) {
            try {
                if (LogUtil.isLoggable(3)) {
                    LogUtil.d("Trying to bind to @".concat(String.valueOf(str)));
                }
